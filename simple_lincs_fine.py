...
from scripts.simple_model import simple_model_feature, simple_model_feature_fine
...
def train_model(model: simple_model_feature_fine, data_train: CustomDataset_1, batch_size = 2, max_epoch = 10, data_test = None):
    ...
    try:
        ...
        tsave(best_model.state_dict(), 'pretrain/simple_lincs_fine.pt')
        print(best_acc)
    tsave(best_model.state_dict(), 'pretrain/simple_lincs_fine.pt')
    print(best_acc)
...
if __name__ == '__main__':
    ...
    for fold in list(folds_index_data_AmbrosM.keys()):
        ...
        'wd': 1e-7,
        'n_feature': 918}
        model_pre = simple_model_feature(hparam=hparam)
        model_pre = model_pre.to(model_pre.device)
        model_pre.load_state_dict(tload('pretrain/simple_lincs.pt'))
        hparam = {
        'hid_dim': 2,
        'mlp_size': [2**7, 2**6],
        'device': device,
        ...
        'wd': 1e-7,
        'nb_feat': 50}
        model = simple_model_feature_fine(model_pretrain=model_pre, hparam=hparam)
