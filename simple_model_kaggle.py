from scripts.simple_model import simple_model
from scripts.data import CustomDataset
import torch 
...
def train_model(model: simple_model, data_train: CustomDataset, batch_size = 2, max_epoch = 10, data_test = None):
...
def predict(dataset: CustomDataset, dataset_train):
    ...
    for id in tqdm(range(num)):
        ...
        drug_feature = dataset.encoder_drug.transform(np.array(drug_name).reshape(-1,1))
        cell_feature = dataset.encoder_cell.transform(np.array(cell_name).reshape(-1,1))
        drugs = torch.tensor(np.vstack(((drug_feature).argmax(axis = 1), (drug_feature).argmax(axis = 1)))).long()
        cells = torch.tensor(np.vstack(((cell_feature).argmax(axis = 1), (cell_feature).argmax(axis = 1)))).long()
        # print(drug_id, cell_id)
        gene_reconstructions = model.predict(
                            drugs, cells
                        )
        gene_reconstruct = dataset_train.reconstruct(gene_reconstructions)
...
if __name__ == '__main__':
    ...
    n_comp = 50 
    dataset = CustomDataset(adata_de, n_comp)
    ...
    for fold in list(folds_index_data_AmbrosM.keys()):
        ...
        dataset_train = CustomDataset(adata_de[id_train], n_comp=n_comp, encoder=dataset, pca_model=None) 
        cell_ohe = dataset_train.encoder_cell.transform( np.array(['NK cells', 'T cells CD4+', 'T cells CD8+', 'T regulatory cells']).reshape(-1,1))
        cell_l_4 = list(cell_ohe.argmax(axis = 1))
        cell_ohe = dataset_train.encoder_cell.transform(np.array(['B cells', 'Myeloid cells']).reshape(-1,1))
        cell_l_2 = list(cell_ohe.argmax(axis = 1))
        dataset_test =  CustomDataset(adata_de[id_test], n_comp=n_comp, encoder=dataset, pca_model=dataset_train.pca_model) 
        hparam = {
        'hid_dim': 2**7,
        'mlp_size': [2**7, 2**6],
        'dim_out': n_comp,
        ...
        'cell_l_4': cell_l_4,
        'cell_l_2': cell_l_2}
        model = simple_model(hparam=hparam)
        ...
        Y_submit_pred_from_one_fold = predict(dataset, dataset_train)
    ...
    df_to_submit.to_csv('results/MLP_50.csv')

# !kaggle competitions submit -c open-problems-single-cell-perturbations -f results/MLP_50.csv -m 'mlp model' 
