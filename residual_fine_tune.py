...
from scripts.simple_model import Simple_residual_feature_fine, Simple_residual_feature
...
def train_model(model: Simple_residual_feature_fine, data_train: CustomDataset_1, batch_size = 2, max_epoch = 10, data_test = None):
...
if __name__ == '__main__':
    ...
    for fold in list(folds_index_data_AmbrosM.keys()):
        ...
        'lr': 1e-3,
        'wd': 1e-7}
        premodel = Simple_residual_feature(hparam=hparam_pre)
        premodel = premodel.to(premodel.device)
        premodel.load_state_dict(tload('pretrain/residual_lincs.pt'))
        hparam = {
        'dim_emb': 2**6,
        'hid': 2**4,
        'nb_layer': 3, 
        ...
        'lr': 1e-4,
        'wd': 1e-7}
        model = Simple_residual_feature_fine(pretrain_model=premodel, hparam=hparam)
    ...
    df_to_submit.to_csv('results/residual_feature.csv')
